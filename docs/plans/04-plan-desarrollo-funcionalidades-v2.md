...
* **El componente "Map Legend" ya existe en Figma** (`Components & Helpers`, nodo `17:74`, ver `estadistica_delicitva` fileKey `NJXIriyDT674hHetseeX0B`) — título "DENSIDAD DELICTIVA", barra de 5 tonos con etiquetas "Mínimo"/"Máximo". *(Corrección 2026-08-07: sí está colocado — hay una instancia en cada uno de los 5 Flow Screens desktop y en Mobile, `x=24` lateral izquierdo, verificado directamente con `findAllWithCriteria` contra el archivo real. La afirmación original de este párrafo, de que "nunca se colocó en ningún Flow Screen", se basó en que no aparecía en una tabla que solo trackeaba instancias de Basemap Switcher — evidencia incompleta, no una verificación directa. Sí sigue siendo cierto que **nunca se implementó en el frontend** — `grep` de "Leyenda\|legend\|Legend" sobre `frontend/src` no encuentra nada — quedó diseñado y colocado en el mockup, pero nunca conectado al código real.)*
