...
**Bug real reportado por el usuario tras el despliegue de lo anterior:** al cambiar a Municipio, la línea departamental quedaba *debajo* de la capa de municipios. Causa raíz: `geometry` (la fuente `regiones`) pasa por `undefined` mientras la geometría de Municipio —bastante más pesada que la de Departamento— todavía está cargando, así que React desmonta y remonta esa fuente/capa; al remontarse, MapLibre la agrega al tope del estilo, por encima de `departamentos-limite` (que nunca se desmontó, porque su geometría ya estaba cacheada). Corregido con un efecto que llama `map.moveLayer(id)` sin `beforeId` (sube la capa al tope) cada vez que cualquiera de las dos geometrías cambia — reafirma el orden sin importar cuál capa termine de montarse después. Verificado con Playwright alternando Departamento/Municipio repetidamente.
