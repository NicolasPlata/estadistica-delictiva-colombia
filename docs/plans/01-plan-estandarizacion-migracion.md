...
## 5. Adenda 2 — Vista Materializada de Rollup (Fase 5, Hito 5.2 del backend)
El profiling del backend (`docs/plans/02-plan-desarrollo-backend.md`) encontró que las agregaciones de `/api/v1/stats/kpi` sobre `estadistica_delictiva` violaban RNF-03 (~1.2s vs. el límite de 300ms) — un problema de volumen, no de índices faltantes. Se agregó `scripts/migrations/0002_vista_materializada_rollup.sql`, que crea `estadistica_rollup` (agregada por todas las dimensiones filtrables, 4,836,275 → 899,265 filas). El backend (`PgStatsRepository`, `PgFiltrosRepository`) consulta esta vista, no la tabla cruda.

**Si se vuelve a ejecutar el ETL o la migración correctiva 0001** (recarga de datos), también hay que correr `REFRESH MATERIALIZED VIEW estadistica_rollup;` — la vista materializada no se actualiza sola, y el backend seguirá sirviendo datos desactualizados desde ella hasta que se refresque.
