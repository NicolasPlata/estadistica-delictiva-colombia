...
*   **Vista materializada de rollup (`estadistica_rollup`):** los índices B-Tree no bastan para agregaciones *sin* filtro (`WHERE 1=1` + `GROUP BY`) — hay que tocar cada fila sin importar el índice. Medido en producción local: las queries de `/api/v1/stats/kpi` sin filtros tardaban 150-400ms cada una sobre 4.8M de filas. `scripts/migrations/0002_vista_materializada_rollup.sql` agrega `estadistica_rollup`, pre-agregada por todas las dimensiones filtrables (899,265 filas, ~5.4x menos), con los mismos índices que la tabla original. `PgStatsRepository`/`PgFiltrosRepository` consultan esta vista. **No se refresca sola** — requiere `REFRESH MATERIALIZED VIEW` manual tras cada re-ejecución del ETL; no se automatizó (triggers/cron) porque el dataset es esencialmente estático entre corridas.
...
*   **Tabla `poblacion_municipal` (Fase 6, RN-11/12/13):** población DANE por municipio/año (2018-2042), clave primaria compuesta `(codigo_dane, anio)` + índice adicional sobre `anio` para el `AVG(poblacion)` agregado a nivel departamental/nacional. Es una tabla pequeña (28.075 filas) sin necesidad de vista materializada — el cálculo de tasa per cápita hace un `JOIN`/consulta independiente contra ella desde `PgStatsRepository::poblacion_promedio`, combinada con el resultado de `map_stats` en Rust (`application::get_map_stats`), no en una única query SQL — mantiene `map_stats` sin cambios (cero riesgo de regresión sobre el RNF-03 ya medido) y deja la fórmula de la tasa como código explícito y testeable sin base de datos. Ver `scripts/migrations/0003_poblacion_indices_y_validacion.sql`.
...
*   **Caché en memoria de datos estáticos:** `metadata/filtros` y `map/geometry/{granularidad}` no dependen de `GlobalFilters` y solo cambian si se re-ejecuta el ETL — se cachean en proceso con `tokio::sync::OnceCell` (compartido entre clones del estado de Axum vía `Arc`), no solo con cabeceras HTTP. El costo real de `map/geometry` (`ST_Union` + `ST_SimplifyPreserveTopology` sobre 1,122 polígonos, 5-9s medido) se paga una única vez precalentando la caché al arrancar el servidor (`main.rs`), nunca en una petición de un usuario real — el "Free-Tier" (siempre activo, ver punto anterior) hace esto viable: en un modelo serverless real, cada cold start volvería a pagar ese costo.
